	...
	<form id="_fom" method="post" action="tomato.cgi">
		...
		<script type="text/javascript">
			...
			for (i = 0; i < tabs.length; ++i) {
				...
				var status = (!eval('vpn'+(i+1)+'up') ? '<small class="text-danger">(停止)</small>' : '<small class="text-success">(运行中)</small>');
				...
				htmlOut += createFormFields([
					...
					{ title: '仅通过用户名认证', indent: 2, name: 'f_vpn_'+t+'_useronly', type: 'checkbox', value: eval( 'nvram.vpn_'+t+'_useronly' ) != 0,
						suffix: '<span class="text-danger" id=\''+t+'_ca_warn_text\'>&nbsp<small>警告：必须定义证书颁发机构.</small></span>' },
					...
					{ title: '服务端在同一子网', name: 'f_vpn_'+t+'_bridge', type: 'checkbox', value: eval( 'nvram.vpn_'+t+'_bridge' ) != 0,
						suffix: '</span><span class="text-danger" id=\''+t+'_bridge_warn_text\'>&nbsp<small>警告：无法桥接不同的子网，正在设置成默认的路由模式.</small></span>' },
